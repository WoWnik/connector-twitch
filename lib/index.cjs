...
var scope = "openid user:read:email";
var defaultMetadata = {
  id: "twitch-universal",
  target: "twitch",
  ...
  formItems: [
    ...
    {
      key: "customConfig",
      type: import_connector_kit.ConnectorConfigFormItemType.Json,
      required: false,
      label: "Custom Config",
      defaultValue: {
        force_verify: true
...
var twitchConfigGuard = import_zod.z.object({
  ...
  customConfig: import_zod.z.record(import_zod.z.string()).optional()
});
var accessTokenResponseGuard = import_zod.z.object({
  ...
  scope: import_zod.z.array(import_zod.z.string()),
...
var getAccessToken = async (config, codeObject) => {
  ...
  if (!result.data.access_token) {
    (0, import_essentials.assert)(result.data.access_token, new import_connector_kit2.ConnectorError(import_connector_kit2.ConnectorErrorCodes.SocialAuthCodeInvalid));
  }
  return {
...
var getTokenResponseAndUserInfo = (getConfig) => async (data, getSession) => {
  ...
  try {
    ...
    const userInfo = { ...userInfoResult.data, rawData };
    return {
      tokenResponse,
      userInfo
    };
