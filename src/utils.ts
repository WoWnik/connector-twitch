...
const accessTokenResponseHandler = async (
  ...
  // Handle scopes as array (Twitch returns an array, normalize to string if needed)
  const scopeArray = result.data.scope;
  const scopeString = Array.isArray(scopeArray) ? scopeArray.join(' ') : scopeArray;
